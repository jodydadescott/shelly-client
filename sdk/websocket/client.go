...
import (
	...
	"github.com/jodydadescott/shelly-client/sdk/websocket/types"
)
...
func (t *Client) SetConfig(ctx context.Context, config *Config) (*bool, error) {
	...
	if config == nil {
		zap.L().Debug("Websocket config is not present and will be disabled")
		config = &Config{}
	} else {
		zap.L().Debug("Websocket config is present")
		config = config.Clone()
	}
